...
export function CustomizeTableDialog({
  ...
  const handleReset = () => {
    ...
    setFeedbackMsg('Columns reset to default view')
    setTimeout(() => setFeedbackMsg(null), 2500)
  }

  const handleSaveAsView = () => {
    setSavedViews((prev) => [...prev, `Custom View (${selectedColumns.length} cols)`])
    setFeedbackMsg('View layout saved to your presets')
    setTimeout(() => setFeedbackMsg(null), 2500)
  }
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-[940px] rounded-[12px] border border-[#d3d5d7] bg-white p-6 shadow-2xl space-y-5 max-h-[92vh] flex flex-col">
        {/* Modal Header */}
        <div className="flex items-start justify-between">
          <div className="space-y-0.5">
            ...
            <p className="text-[14px] text-[#6f777f]">
              Choose which columns appear in the table and arrange their order.
            </p>
          </div>
          <button
            type="button"
        ...
        {feedbackMsg && (
          <div className="rounded-[6px] bg-[#dfefe8] border border-[#9cdabd] px-3 py-1.5 text-xs font-medium text-[#17b26a] flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-5 flex-1 overflow-y-auto pr-1">
          ...
          <div className="md:col-span-5 flex flex-col gap-4 border-t md:border-t-0 md:border-l border-[#d3d5d7] pt-4 md:pt-0 md:pl-5">
            <div className="space-y-1">
              <h3 className="text-[18px] font-semibold text-[#1f2327]">Column Order</h3>
              <p className="text-[16px] font-medium text-[#1f2327]">{selectedColumns.length} columns selected</p>
              <p className="text-[13px] text-[#6f777f] leading-snug">
                Drag and drop to arrange columns as they'll appear in the table.
              </p>
            </div>

            <div className="flex-1 flex flex-col rounded-[8px] border border-[#d3d5d7] bg-white overflow-hidden shadow-2xs">
              <div className="bg-[#f8f9fa] border-b border-[#d3d5d7] px-3.5 py-2.5">
                <span className="text-[14px] font-semibold text-[#1f2327]">Visible Columns</span>
              </div>

              <div className="divide-y divide-[#d3d5d7] overflow-y-auto max-h-[380px]">
        ...
        {/* Footer Actions */}
        <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-[#d3d5d7]">
          {/* Bottom Left Buttons: Reset & Save as View */}
          <div className="flex items-center gap-2.5">
            <button
              type="button"
              onClick={handleReset}
            ...
            <button
              ...
              className="h-[36px] px-3.5 rounded-[8px] border border-[#d3d5d7] bg-white text-[14px] font-medium text-[#1f2327] hover:bg-[#eff1f3] transition-colors cursor-pointer ant-wave-btn flex items-center gap-1.5"
            >
              <Bookmark className="size-3.5 text-[#6f777f]" />
